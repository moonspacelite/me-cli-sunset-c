...
// Trik Rahasia 1: Header mundur 5 menit (-300 detik)
...
// Trik Rahasia 2: Signature menggunakan waktu saat ini
...
static char* generate_ax_fingerprint(const char* msisdn) {
    ...
    char plain[256]; 
    snprintf(plain, sizeof(plain), "samsung|SM-N935F|en|720x1540|GMT07:00|192.168.1.1|1.0|Android 13|%s", msisdn);
    
    unsigned char key[32] = {0}; 
    memcpy(key, key_str, strlen(key_str) > 32 ? 32 : strlen(key_str));
    unsigned char iv[16] = {0};
    
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new(); 
    EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, key, iv);
    
    unsigned char ct[256]; int len1, len2;
    EVP_EncryptUpdate(ctx, ct, &len1, (unsigned char*)plain, strlen(plain));
