...
void handle_payment_menu(const char* B_CIAM, const char* B_API, const char* B_AUTH, const char* UA, const char* API_KEY, const char* XDATA_KEY, const char* X_API_SEC, const char* ENC_FIELD_KEY, cJSON* tokens_arr, const char* opt_code, int price, const char* name, const char* conf, const char* p_for, cJSON* bm_info, int* goto_main_flag) {
    while (1) {
        ...
        if (strcmp(pay_choice, "00") == 0) { break; } 
            ...
            char use_decoy_str[10]; printf("Use decoy package? (y/n): "); if(!fgets(use_decoy_str, sizeof(use_decoy_str), stdin)) continue;
            char n_times_str[10]; printf("Enter number of times to purchase (e.g., 3): "); if(!fgets(n_times_str, sizeof(n_times_str), stdin)) continue;
            char delay_str[10]; printf("Enter delay between purchases in seconds (e.g., 25): "); if(!fgets(delay_str, sizeof(delay_str), stdin)) continue;
            ...
            printf("Apakah anda yakin ingin melanjutkan pembelian? (y/n): ");
            char conf_str[10];
    ...
    while (1) {
        ...
        if (strcmp(choice, "1") == 0) {
            while (1) {
                ...
                if (fgets(cmd, sizeof(cmd), stdin) != NULL) {
                    ...
                    if (strcmp(cmd, "00") == 0 || strcmp(cmd, "99") == 0) break;
                        ...
                        char new_num[32]; printf("\nMasukkan Nomor XL (misal 62819...): ");
                        if (fgets(new_num, sizeof(new_num), stdin) != NULL) {
                            new_num[strcspn(new_num, "\n")] = 0; clean_input_string(new_num);
                            printf("\n[*] Mengirim OTP ke %s...\n", new_num);
                            cJSON* otp_res = request_otp(B_CIAM, B_AUTH, UA, new_num); if (otp_res) cJSON_Delete(otp_res);
                            
                            char otp_code[10]; printf("Masukkan 6-digit OTP: ");
                            if (fgets(otp_code, sizeof(otp_code), stdin) != NULL) {
                                ...
                                if (login_res && cJSON_GetObjectItem(login_res, "refresh_token")) {
                                    ...
                                    authenticate_and_fetch_balance(tokens_arr, B_CIAM, B_API, B_AUTH, UA, API_KEY, XDATA_KEY, X_API_SEC);
                                    printf("\n[+] LOGIN BERHASIL!\n");
                                } else { printf("\n[-] Login Gagal. Pastikan OTP benar.\n"); }
            ...
            while (1) {
                ...
                printf("Input package number to view detail.\nInput del <package number> to unsubscribe from a package.\n00. Kembali ke menu utama\nChoice: ");
                char p_cmd[64];
                if (fgets(p_cmd, sizeof(p_cmd), stdin) != NULL) {
                    ...
                    if (strncmp(p_cmd, "del ", 4) == 0) {
                        int del_idx = atoi(p_cmd + 4) - 1;
                        if (del_idx >= 0 && del_idx < active_pkg_count) {
                            printf("Are you sure you want to unsubscribe from package %d. %s? (y/n): ", del_idx + 1, active_pkgs[del_idx].name);
                            char conf[10];
                            if (fgets(conf, sizeof(conf), stdin) && (conf[0] == 'y' || conf[0] == 'Y')) {
                                printf("Unsubscribing from package %s...\n", active_pkgs[del_idx].name);
                                ...
                                if (u_res && cJSON_GetObjectItem(u_res, "code") && strcmp(cJSON_GetObjectItem(u_res, "code")->valuestring, "000") == 0) {
                                    printf("Successfully unsubscribed from the package.\n");
                                } else {
                                    printf("Failed to unsubscribe from the package.\n");
                                }
                                if (u_res) {
                            ...
                            } else { printf("Unsubscribe cancelled.\n"); }
                            printf("\nTekan Enter untuk melanjutkan..."); int temp_c = getchar(); (void)temp_c;
                        } else {
